...
void usage()
{
  fprintf (stdout,
     ...
     " -v               verbose messages\n"
     " -h               print help text\n",
     DSAX_DEFAULT_BLOCK_KEY ,
     DSAX_DEFAULT_BLOCK_KEY2 , 
     DSAX_DEFAULT_UDPDB_PORT);
}
...
{
  if (ctx->verbose > 1)
    multilog (ctx->log, LOG_INFO, "dsax_udpdb_init_receiver()\n");
...
{
  if (ctx->verbose)
    multilog(ctx->log, LOG_INFO, "dsaX_udpdb_prepare()\n");
...
{
  if (ctx->verbose)
    multilog (ctx->log, LOG_INFO, "dsaX_udpdb_reset_receiver()\n");
 ...
 */
time_t dsaX_udpdb_start (udpdb_t * ctx, char * obs_header)
{
...
int dsaX_udpdb_open_buffer (udpdb_t * ctx)
{
  ...
  if (ctx->verbose > 1)
    multilog (ctx->log, LOG_INFO, "dsaX_udpdb_open_buffer()\n");
 ...
 * datablocks
 */
void * dsaX_udpdb_receive_obs (void * arg)
{
  ...
  if (ctx->verbose)
    multilog(log, LOG_INFO, "dsaX_udpdb_receive_obs()\n");
  ...
  while (!quit_threads && !stop_pending) 
  {
    ...
    if (ctx->sock->have_packet) 
    {
      ...
      // decode sequence number of packet
      seq_no = UINT64_C (0);
      for (i = 1; i < 8; i++ )
      {
        tmp = b[7-i];
      ...
      //printf("Seq number %"PRIu64"\n",(seq_no));
      // Use continue to only start packet capture when packet 1e6 arrives.
      if (seq_no == start_packet) vrstart=1;
      if (vrstart == 0) continue;
      ...
      if (ctx->capture_started)
      {
        seq_byte = (ctx->num_inputs * seq_no * UDP_DATA);
        if (ctx->verbose > 2)
      ...
      if ((ctx->block_count >= ctx->packets_per_buffer) || (temp_idx >= temp_max))
      {
        ...
        if (dsaX_udpdb_new_buffer (ctx) < 0)
        {
          multilog(ctx->log, LOG_ERR, "receive_obs: dsaX_udpdb_new_buffer failed\n");
          thread_result = -1;
          pthread_exit((void *) &thread_result);
        }
    ...
    if (stop_byte)
    {
      if (seq_byte >= stop_byte)
      {
        ...
        if (dsaX_udpdb_close_buffer (ctx, bytes_just_written, 1) < 0)
        {
          multilog(ctx->log, LOG_ERR, "receive_obs: dsaX_udpdb_close_hdu failed\n");
          thread_result = -1;
          pthread_exit((void *) &thread_result);
        }
        stop_byte = 0;
...
int dsaX_udpdb_close_buffer (udpdb_t * ctx, uint64_t bytes_written, unsigned eod)
{
  ...
  if (ctx->verbose && stop_pending || ctx->verbose > 1)
    multilog (ctx->log, LOG_INFO, "dsaX_udpdb_close_buffer(%"PRIu64", %d)\n", bytes_written, eod);
...
int dsaX_udpdb_new_buffer (udpdb_t * ctx)
{
  ...
  if (ctx->verbose > 1)
    multilog (ctx->log, LOG_INFO, "dsaX_udpdb_new_buffer()\n");

  if (dsaX_udpdb_close_buffer (ctx, ctx->hdu_bufsz, 0) < 0)
  {
    multilog (ctx->log, LOG_INFO, "new_buffer: dsaX_udpdb_close_buffer failed\n");
    return -1;
  }

  if (dsaX_udpdb_open_buffer (ctx) < 0) 
  {
    multilog (ctx->log, LOG_INFO, "new_buffer: dsaX_udpdb_open_buffer failed\n");
    return -1;
  }
...
int main (int argc, char **argv)
{
  ...
  int inc_port = DSAX_DEFAULT_UDPDB_PORT;

  /* multilog output port */
  int l_port = DSAX_DEFAULT_PWC_LOGPORT;
  ...
  while ((arg=getopt(argc,argv,"b:c:f:i:k:l:n:p:t:vh")) != -1) {
    switch (arg) {
  ...
  if (header_file != NULL)
  {
    ...
    // read the ASCII DADA header from the file. fileread.c is just an fread.
  ...
  // won't run in background as daemon. 
  log = multilog_open ("dsax_udpdb_thread", 0);
  multilog_add (log, stderr);
  udpdb.log = log;

  // initialize the data structure
  multilog (log, LOG_INFO, "main: dsaX_udpdb_init_receiver()\n");
  multilog (log, LOG_INFO, "main: starting on start_packet %llu\n",start_packet);
  ...
  // print keys
  printf("Header block key %d, data block key %d\n",hdu->header_block_key,hdu->data_block_key);
  ...
  if (verbose)
    multilog(log, LOG_INFO, "main: dsaX_udpdb_prepare()\n");
  ...
  {
    if (verbose)
      multilog(log, LOG_INFO, "main: dsaX_udpdb_reset_receiver()\n");
    ...
    {
      if (verbose)
        multilog(log, LOG_INFO, "main: dsaX_udpdb_start()\n");
      time_t utc = dsaX_udpdb_start (&udpdb, obs_header);
      if (utc == -1 ) {
    ...
    udpdb.bytes_to_acquire = 1000 * 1000 * 1000 * (int64_t) nsecs;
    ...
    if (verbose)
      multilog(log, LOG_INFO, "starting dsaX_udpdb_receive_obs thread\n");
    rval = pthread_create (&receiving_thread_id, 0, (void *) dsaX_udpdb_receive_obs , (void *) &udpdb);
    if (rval != 0) {
      multilog(log, LOG_INFO, "Error creating dsaX_udpdb_receive_obs thread: %s\n", strerror(rval));
      return -1;
    }

    if (verbose) 
      multilog(log, LOG_INFO, "joining dsaX_udpdb_receive_obs thread\n");
    pthread_join (receiving_thread_id, &result);
    multilog(log, LOG_INFO, "joined dsaX_udpdb_receive_obs thread\n");
  ...
  // clean up memory 
  if ( dsaX_udpdb_destroy_receiver (&udpdb) < 0) 
