...
void brain(std::vector<hd_byte> &data, int nsamps, int nbin, float SNR, float dm, float* S1, float* S2);

void brain(std::vector<hd_byte> &data, int nsamp, int nbin, float SNR, float dm, float* S1, float* S2) {
  ...
  int nsamps = 2*nsamp;
  float dm_max = 0.9*1.31072e-4*nsamps/7.6e-4;
  //float dm_max = 2.*dm;
  ...
  // double data
  hd_byte* ddata = (hd_byte *)malloc(sizeof(hd_byte)*2*nchans*nsamps);
  std::copy(data.begin(), data.end(), ddata);
  std::copy(data.begin(), data.end(), ddata+nchans*nsamps);
  
  // dedisperse
  const dedisp_byte* in = (dedisp_byte*)(&ddata[0]);
...
hd_error hd_execute(hd_pipeline pl,
  ...
  double tmjd;
  int doCrab=0;
  fcrab=fopen("/usr/local/heimdallr/Share/crab_mjds.dat","r");
  ...
  if ( pl->params.coincidencer_host != NULL && pl->params.coincidencer_port != -1 )
  {
   ...
   for( hd_size i=0; i<h_group_peaks.size(); ++i ) {
     ...
     if (h_group_peaks[i]<7.0202 || h_group_peaks[i]>7.020203) {
       ...
       if ((samp_idx>100000) && ((h_group_dms[i] > 100. && notrig==0 && h_group_peaks[i]>6.45) || (h_group_dms[i]>55.0 && h_group_dms[i]<58.5 && notrig==0 && doCrab && h_group_filter_inds[i]<8))) {

	 // find S1 and S2
	 // get data from pulse
	 start_timer(clean_timer);
	 s1 = h_group_begins[i]-50;
	 if (s1<0) s1=0;
	 s2 = h_group_ends[i]+int((0.000761*h_group_dms[i])/pl->params.dt)+50+(int)(h_group_filter_inds[i]);
   ...
   if (h_group_peaks.size()>0 && maxI!=-1 && h_group_peaks.size()<7) {
