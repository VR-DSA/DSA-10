...
#include "dsaX_spectrometer_udpdb_thread.h"
...
void * dsaX_udpdb_receive_obs (void * arg)
{
  ...
  while (!quit_threads && !stop_pending) 
  {
    ...
    if (ctx->sock->have_packet) 
    {
      ...
      // decode sequence number of packet - TEST
      seq_no = UINT64_C (0);
      for (i = 1; i < 8; i++ )
      {
        tmp = b[7-i];
...
int main (int argc, char **argv)
{
  ...
  while ((arg=getopt(argc,argv,"b:c:f:i:k:l:p:t:vh")) != -1) {
    switch (arg) {
  ...
  multilog (log, LOG_INFO, "main: starting at packet %llu\n",start_packet);
  ...
  // print keys
  multilog(log, LOG_INFO, "Header block key %d, data block key %d\n",hdu->header_block_key,hdu->data_block_key);
